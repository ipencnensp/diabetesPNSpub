...
pns2013.1 <- import("data/PNSIBGE_Rpkg/pns2013microdata_get_pns20230505.csv")
```
...
```{r tblcaract}
tbls_caract <- rbind(tbl_fxetaria, tbl_raceskin, tbl_fxesc, tbl_imcclass, tbl_hyperten)
```

```{r tblgt, include = FALSE}
tbls_caract %>% 
  rename(` `=caract) %>%
  gt() %>%
 tab_header(
...
```{r}
com.diabetes.wt.df <- subset(pns_design_srvyr13, diabetes == 1 | diabetes == 2)
...
```{r eval = FALSE}
saveRDS(diabetes2013_gt, file = "diabetes2013_gt_20230505.rds", compress = TRUE)
```
...
```{r eval = FALSE}
saveRDS(diabetes2019_gt, file = "diabetes2019_gt_20230505.rds", compress = TRUE)
```
...
```{r eval = FALSE}
saveRDS(diab1319.df, file = "diab1319_20230505.rds", compress = TRUE)
```
...
fxetariascut %>% flextable() %>% autofit()
```
