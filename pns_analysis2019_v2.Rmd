...
```{r criavars1}
#load("~/datasets/IBGE/PNS/2019/pns2019_dwnld20210923.rdata")
#pns2019 <- readRDS("~/datasets/IBGE/PNS/PNSIBGE_Rpkg/pns2019microdata_dnld20221122.RDS")
...
#  mutate(diabetes2 = if_else(gender == "Women" & q03002 == "1" & q03001 == "1", 2, 
#                       if_else(gender == "Women" & q03002 == "2" & q03001 == "1", 1,
#                         if_else(gender == "Men" & q03001 == "1", 1,
#                           if_else(gender == "Men" & q03001 == "2", 3,
#                             if_else(gender == "Women" & q03002 == "2" & q03001 == "2", 3, 4))))))
...
```{r}
svytable(~q03001, subset(pns_design_srvyr, c008 >= 18 & gender == "Men"), addNA = TRUE)
```

```{r}
svytable(~q03001, subset(pns_design_srvyr, c008 >= 18 & gender == "Women"), addNA = TRUE)
```


```{r}
svytable(~q03001, subset(pns_design_srvyr, c008 >= 18 & gender == "Women" & q03002 == "2" & q03001 == "1"), addNA = TRUE)
```

```{r}
svytable(~q03001, subset(pns_design_srvyr, c008 >= 18 & gender == "Women" & q03002 == "1" & q03001 == "1"), addNA = TRUE)
```
...
pns_design_srvyr <- pns_design_srvyr %>%
  mutate(q03001 = as.character(q03001),
        q03002 = as.character(q03002)) %>%
  mutate(diabetes = case_when(gender == "Women" & q03002 == "1" ~ 2, # Gravidez -> No
                               q03001 == "1" ~ 1, #"Yes",
                               q03001 == "2" ~ 3, #"No",
                               q03001 == "9" ~ 4, #ignorado
                               TRUE ~ 4, #"Missing"
...
```{r}
svytable(~diabetes, subset(desPNSQ18, gender == "Men"), addNA = TRUE)
svytable(~diabetes, subset(desPNSQ18, gender == "Men"), addNA = TRUE, Ntotal = 100)
```
...
```{r}
svytable(~diabetes, subset(desPNSQ18, gender == "Women"), addNA = TRUE)
svytable(~diabetes, subset(desPNSQ18, gender == "Women"), addNA = TRUE, Ntotal = 100)
```
...
```{r}
prat_ativ_bmi_ndiab <- svyby(~one, ~prat_ativ + imcclass2, subset(df, diabetes != 1), svytotal, vartype = "ci",  Ntotal = 100,multicore = TRUE)
prat_ativ_bmi_ndiab.df <- prat_ativ_bmi_ndiab %>% as_tibble() %>%
  mutate(one = round(one,0),
         `(%)`=round(one/sum(one)*100,1),
         ci_l_perc = round(ci_l/sum(one)*100,1),
         ci_u_perc = round(ci_u/sum(one)*100,1)) %>%
  select(-ci_l, -ci_u)
prat_ativ_bmi_ndiab.df %>% flextable() %>% autofit()
...
```{r}
prat_ativ_bmi_diab <- svyby(~one, ~prat_ativ + imcclass2, subset(df, diabetes == 1), svytotal, vartype = "ci", Ntotal = 100, multicore = TRUE)
prat_ativ_bmi_diab.df <- prat_ativ_bmi_diab %>% as_tibble() %>%
  mutate(one = round(one,0),
         `(%)` = round(one/sum(one)*100,1),
         ci_l_perc = round(ci_l/sum(one)*100,1),
         ci_u_perc = round(ci_u/sum(one)*100,1)) %>%
  select(-ci_l, -ci_u)
prat_ativ_bmi_diab.df %>% flextable() %>% autofit()
```

```{r}
prat_ativ_hypert_ndiab <- svyby(~one, ~prat_ativ + Hypertension, subset(df, diabetes != 1 & Hypertension == "Yes"), svytotal, vartype = "ci",  Ntotal = 100,multicore = TRUE)
prat_ativ_hypert_ndiab.df <- prat_ativ_hypert_ndiab %>% as_tibble() %>%
  mutate(one = round(one,0),
         `(%)`=round(one/sum(one)*100,1),
         ci_l_perc = round(ci_l/sum(one)*100,1),
         ci_u_perc = round(ci_u/sum(one)*100,1)) %>%
  select(-ci_l, -ci_u)
prat_ativ_hypert_ndiab.df %>% flextable() %>% autofit()

```

```{r}
bmi_hypert_ndiab <- svyby(~one, ~Hypertension + imcclass2, subset(df, diabetes != 1  & Hypertension == "Yes"), svytotal, vartype = "ci",  Ntotal = 100,multicore = TRUE)
bmi_hypert_ndiab.df <- bmi_hypert_ndiab %>% as_tibble() %>%
  mutate(one = round(one,0),
         `(%)`=round(one/sum(one)*100,1),
         ci_l_perc = round(ci_l/sum(one)*100,1),
         ci_u_perc = round(ci_u/sum(one)*100,1)) %>%
  select(-ci_l, -ci_u)
bmi_hypert_ndiab.df %>% flextable() %>% autofit()
...
```{r eval = FALSE}
saveRDS(x, file = "distribuicaoNAsrisco2019.rds", compress = TRUE)
```
...
pontos_genero_2019.df <- pontos_genero2019 %>% 
  filter(risco == 1) %>%
  mutate(one = round(one,0),
         `(%)`=round(one/sum(one)*100,1), 
         ci_l_perc = round(ci_l/sum(one)*100,1), 
...
```{r}
pontos_pregnancy2019 <- svyby(~one, ~risco + gender, subset(df, diabetes == 2), svytotal, vartype = "ci", Ntotal = 1, multicore = TRUE)
pontos_pregnancy2019
```


```{r}
pontos_pregnancy2019.df <- pontos_pregnancy2019 %>%
  mutate(one = round(one,0),
         `(%)`=round(one/sum(one)*100,1), 
         ci_l_perc = round(ci_l/sum(one)*100,1), 
         ci_u_perc = round(ci_u/sum(one)*100,1)) %>%
...
pontos_genero_2019.df <- as_tibble(pontos_genero_2019.df) %>%
  add_column(Classe = "Gender") %>%
  rename(Nome = gender, n2019 = N) %>%
  mutate(Nome = as.character(Nome))
...
pontos_2019.df <- bind_rows(pontos_genero_2019.df, pontos_fxeta2_2019.df,pontos_race_2019.df,pontos_fxesc_2019.df,pontos_imc_2019.df,pontos_hypert_2019.df)
```
...
Proporção de pessoas que nunca tiveram diagnóstico por UF

```{r tab.cap = "PNS 2019 - Proporção de pessoas que nunca tiveram diagnóstico, por UF"}

pontos_uf_semdiag_2019 <- svyby(~one, ~uf, subset(df, diabetes == 4 & !is.na(risco)), svytotal, vartype = "ci", Ntotal = 1, multicore = TRUE)
pontos_uf_semdiag_2019.df <- pontos_uf_semdiag_2019 %>%
  #filter(risco == 1) %>% 
  left_join(pop_ufpns2019, by = "uf") %>%
  mutate(one = round(one,0), popul = round(popul,0),
         `(%)`=round(one/popul*100,1), 
         ci_l_perc = round(ci_l/popul*100,1), 
         ci_u_perc = round(ci_u/popul*100,1)) %>%
...
pontos_uf_semdiag_2019.df %>% arrange(desc(`(%)`)) %>%
 # adorn_totals() %>%
  flextable() %>% autofit()
```

```{r}
pontos_uf_semdiag_2019.df <- pontos_uf_semdiag_2019.df %>%
    mutate(Regiao = fct_collapse(uf, 
                `Norte` = c("Rondonia","Acre","Amazonas","Roraima","Para", "Amapa","Tocantins"),
                `Nordeste` = c("Maranhao", "Piaui", "Ceara", "Rio Grande do Norte", "Paraiba","Pernambuco", "Alagoas","Sergipe","Bahia"),
                `Sudeste` = c("Minas Gerais", "Espirito Santo","Rio de Janeiro", "Sao Paulo"), 
...
```{r}
pontos_uf_semdiag_2019.df %>%
  group_by(cut(pontos_uf_semdiag_2019.df$`(%)`,breaks = c(0.1,4.9,9.9,14.9,19.9,Inf),
                      labels = c("0-4.9","5.0-9.9","10-14.9","15-19.9","20+"))) %>%
  arrange(`(%)`)
...
```{r}
pontos_reg_semdiag_2019 <- svyby(~one, ~regiao, subset(df, diabetes == 4 & !is.na(risco)), svytotal, vartype = "ci",  Ntotal = 100,multicore = TRUE)
pontos_reg_semdiag_2019.df <- pontos_reg_semdiag_2019 %>% 
  left_join(pop_regpns2019, by = "regiao") %>%
  mutate(one = round(one,0),
         `(%)`=round(one/popul*100,1), 
         ci_l_perc = round(ci_l/popul*100,1), 
         ci_u_perc = round(ci_u/popul*100,1)) %>%
...
pontos_reg_semdiag_2019.df <- pontos_reg_semdiag_2019.df %>% arrange(desc(`(%)`))
pontos_reg_semdiag_2019.df <- pontos_reg_semdiag_2019.df %>%
...
pontos_reg_semdiag_2019.df %>%
#  adorn_totals() %>%
  flextable() %>% autofit()
```

```{r eval = FALSE}
saveRDS(pontos_reg_semdiag_2019.df, "pontos_regiao_semdiag2019.rds")
```
...
```{r eval = FALSE}
saveRDS(pontos_2019.df, file = "pontos_2019.rds", compress = TRUE)
```
...
https://www.apheo.ca/standardization-of-rates
https://www.statcan.gc.ca/en/dai/btd/asr
https://stat.ethz.ch/pipermail/r-help/2012-August/321495.html
-->
...
#projecaopop <- "/home/mario/datasets/IBGE/Populacao/PROJECOES_2013_POPULACAO.xls"
...
#projpopsimples <- read_excel(projecaopop,  sheet = "BR IDADES SIMPLES", skip = 195,n_max = 92) # para projecao 2013
projpopsimples <- read_excel(projecaopop,  sheet = "BR", skip = 195,n_max = 92) # para projecao 2018
```
...
(x,y] => o "x" não está incluso no intervalo mas o "y" está.

-->

Penso que um ponto a considerar na padronização é que na pesquisa temos apenas pessoas acima de determinada idade, 18 anos na PNS2013 e 15 anos na PNS2019.

```{r}
svytable(~age, pns_design_srvyr) %>% as_tibble() %>%
  adorn_totals() %>%
  adorn_percentages(denominator = "col")
```

Para a PNS 2019, como temos pessoas a partir de 15 anos, precisei colocar uma faixa 14.9-17.9 para incluir essas pessoas.
...
```{r}
getPrevalenced <- function(over) {
  group_vars <- syms(over)
  svystandardize(pns_design_srvyr, by = ~ age, over = make.formula(over),
                 population = popfxetarias19_f, 
                 excluding.missing = make.formula(c(over, "age", "diabetes"))) %>% 
    #filter(idade >= 18) %>%
    group_by(!!!group_vars) %>%
    summarize(n = unweighted(n()),
              pct = survey_mean(diabetes == "Yes")) %>% 
...
```{r eval=FALSE}
pns_design_srvstd <- svystandardize(pns_design_srvyr, by = ~ age, over = ~1, 
                                    population = popfxetarias19_f)

df_std <- svystandardize(df, by = ~ age, over = ~1, population = popfxetarias19_f)
```



```{r eval=FALSE}
todos18std <- svytable(~one, subset(pns_design_srvstd, c008>=18))
```


```{r eval=FALSE}
diabetesstd.df <- svyby(~one, ~diabetes, subset(pns_design_srvstd, idade >= 18), svytotal, vartype = c("se","ci"), keep.names = F)
diabetesstd.df <- diabetesstd.df %>% 
   mutate(Perc = round(one/todos18std*100,2),PercSE = round(se/todos18std*100,2), Perci_l = round(ci_l/todos18std*100,2), Perci_u = round(ci_u/todos18std*100,2)) %>%
   rename(Diabetes_std=diabetes,Population=one,SE=se) %>%
  select(Diabetes_std, Population, Perc, PercSE, Perci_l, Perci_u)
bind_rows(diabetesstd.df, tibble(Population = round(sum(diabetesstd.df$Population),1),  Perc = round(sum(diabetesstd.df$Perc),1))) %>% flextable() %>% autofit()
```


```{r eval=FALSE}
fxetaria2std.df <- svyby(~one, ~fxetaria2, subset(pns_design_srvstd, idade >= 18), svytotal, vartype = c("se","ci"), keep.names = F)
fxetaria2std.df <- fxetaria2std.df %>% 
   mutate(Perc = round(one/todos18std*100,2),PercSE = round(se/todos18std*100,2), Perci_l = round(ci_l/todos18std*100,2), Perci_u = round(ci_u/todos18std*100,2)) %>%
   rename(Age_std=fxetaria2,Population=one,SE=se) %>%
  select(Age_std, Population, Perc, PercSE, Perci_l, Perci_u)
bind_rows(fxetaria2std.df, tibble(Population = round(sum(fxetaria2std.df$Population),1),  Perc = round(sum(fxetaria2std.df$Perc),1))) %>% flextable() %>% autofit()
```

```{r eval = FALSE}
svytable(~one, subset(pns_design_srvstd, c008>=18))
```


```{r eval=FALSE}
getPrevalence <- function(variavel, dados) {
 ptos_temp <- svyby(~one, by = make.formula(c("risco", variavel)), design = dados, svytotal, vartype = "ci",  Ntotal = 100,multicore = TRUE)
ptos_temp <- ptos_temp %>% filter(risco == 1) %>% 
  mutate(one = round(one,0),
         `(%)`=round(one/sum(one)*100,1), 
         ci_l_perc = round(ci_l/sum(one)*100,1), 
         ci_u_perc = round(ci_u/sum(one)*100,1)) %>%
...
```{r eval=FALSE}
dados.df <- subset(df_std, diabetes != 1 & !is.na(risco))
```

### Risco por Faixa Etária age standardized

```{r eval=FALSE}
diabstdfxet19 <- getPrevalence("fxetaria2", dados.df)
diabstdfxet19 %>%
  adorn_totals() %>%
  flextable() %>% autofit()
```
...
### Risco por IMC age standardized

```{r eval=FALSE}
diabstdimc19 <- getPrevalence("imcclass2", dados.df)
diabstdimc19 %>%
  adorn_totals() %>%
  flextable() %>% autofit()
```

### Risco por Raça, Cor da Pele age standardized

```{r eval = FALSE}
diabstdraceskin <- getPrevalence("RaceSkinColor", dados.df)
diabstdraceskin %>%
  adorn_totals() %>%
  flextable() %>% autofit()
```

### Risco por Hipertensão age standardized

```{r eval = FALSE}
diabstdhypert <- getPrevalence("Hypertension", dados.df)
diabstdhypert %>%
  adorn_totals() %>%
  flextable() %>% autofit()
```

### Risco por Escolaridade age standardized

```{r}
diabstdfx_esc <- getPrevalence("fx_esc", dados.df)
diabstdfx_esc %>%
  adorn_totals() %>%
  flextable() %>% autofit()
```
